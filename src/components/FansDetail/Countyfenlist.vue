<template>
  ...
  >
    <div class="channelfan" >
      <div class="title">
        <router-link to="/home/fans/channelfan" class="fa icon-left back" tag="i"></router-link>
        <span>{{chaFansDetail.county_name}}</span>
      </div>
      <div class="content">
        <div class="data" id="directfan">
          <ul>
            <li class="list" v-for="list in chaFansDetailistInfo.fen_date_list"  @click="openDetail(list)">
      ...
      getData(){
        $.ajax({
          ...
          data:{
            ...
            county_id:this.chaFansDetail.county_id
          },
          success:(res) => {
            console.log(res);
            if(res.status==-1){
