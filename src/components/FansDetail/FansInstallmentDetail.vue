...
<transition  
  ...
  <div class="fansdetail">
    <div class="title">
      <router-link to="/home/fans/Countyfenlist" class="fa icon-left back"></router-link>
      <span>详情</span>
    </div>
    <div class="main">
      ...
      <div class="main_top">
        <div class="mtitle">
          <span>{{countylistDate.date | changDate}}</span>月新增粉丝数
        </div>
        <div class="num">{{countylistDate.usercount}}</div>
        <div class="type">
          <p class="t_num">已激活：<span>{{fanlistinfo.isact_num ?fanlistinfo.isact_num:0}}</span>人</p>
          <p class="t_num">未激活：<span>{{fanlistinfo.noact_num ? fanlistinfo.noact_num:0}}</span>人</p>
        </div>
      </div>
      <div class="content" >
        <div id="fansdetail" class="data" >
          <ul>
            <li class="list" v-for="list in fanlistinfo.fen_list" @click='getDetail(list)'>
...
export default {
      ...
      getData(){
        $.ajax({
          ...
          data:{
            ...
            county_id:this.countylistDate.county_id,
            group:2,
          ...
          success:(res) => {
            ...
            if(res.status==-1){
              ...
              this.$store.dispatch('getFanlistInfo',res);
            }
             setTimeout(function(){ 
    ...
    computed:{
      fanlistinfo:function(){
        return this.$store.state.fanlistinfo;
      },
